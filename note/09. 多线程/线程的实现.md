...
`thread.join()` 调用时, 因为 `join`是个 `synchronized` 方法, 因此主线程会持有 `thread` 对象锁, `isAlive` 方法是由 `thread` 对象调用的用来查看 `thread` 对象是否运行完成(即进入终止态). 如果运行结束则直接退出方法; 否则主线程执行 `thread.wait` 方法, 主线程释放 `thread` 对象, 同时主线程被移动至 `thread` 对象的等待队列.
